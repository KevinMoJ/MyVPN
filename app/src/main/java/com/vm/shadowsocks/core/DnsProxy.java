...
public class DnsProxy implements Runnable {
    ...
    private boolean dnsPollution(byte[] rawPacket, DnsPacket dnsPacket) {
        if (dnsPacket.Header.QuestionCount > 0) {
            ...
            if (question.Type == 1) {
                ...
                if (ProxyConfig.Instance.needProxyForDnsQuery(question.Domain, realIP)) {
    ...
    private void OnDnsResponseReceived(IPHeader ipHeader, UDPHeader udpHeader, DnsPacket dnsPacket) {
        ...
        if (state != null) {
            ...
            if (ProxyConfig.IS_DEBUG){
                ...
                System.out.printf("DNS receive source: %s:%d, remote: %s:%d, answer: %s\n",
                        ...
                        dnsPacket.Resources.length > 0 ? dnsPacket.Resources[0].Domain : "null");
            }
    ...
    private boolean interceptDns(IPHeader ipHeader, UDPHeader udpHeader, DnsPacket dnsPacket) {
        ...
        if (question.Type == 1) {
            if (ProxyConfig.Instance.needProxyForDnsQuery(question.Domain, getIPFromCache(question.Domain))) {
                int fakeIP = getOrCreateFakeIP(question.Domain);
                ...
                udpHeader.setTotalLength(8 + dnsPacket.Size);
                if (ProxyConfig.IS_DEBUG) {
                    System.out.printf("interceptDns ipSourceIP: %s, ipDestinationIP: %s, udpSourcePort: %d, udpDestinationPort %d\n",
