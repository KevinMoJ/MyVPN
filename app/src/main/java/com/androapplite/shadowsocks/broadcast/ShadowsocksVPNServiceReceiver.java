        ...
        if(intent != null) {
            ...
            switch (action){
                ...
                case Action.CONNECTING:
                    Firebase.getInstance(context).logEvent( "VPN状态", "开始连接");
                    if(duration > 0){
                        Firebase.getInstance(context).logEvent( "VPN计时", "使用", duration);
                    }
                    break;
                case Action.CONNECTED:
                    ...
                    Firebase.getInstance(context).logEvent( "VPN状态", "连接成功", "连续错误" + errorCount);
                    sharedPreferences.edit()
                            .putBoolean(SharedPreferenceKey.FIRST_CONNECT_SUCCESS, true)
                            .remove(SharedPreferenceKey.CONNECT_ERROR_COUNT)
                            .commit();
                    if(duration > 0){
                        Firebase.getInstance(context).logEvent( "VPN计时", "连接", duration);
                    }
                    VpnStatusMonitorService.startService(context);
                    break;
                case Action.STOPPING:
                    Firebase.getInstance(context).logEvent( "VPN状态", "开始断开");
                    if(duration > 0){
                        Firebase.getInstance(context).logEvent( "VPN计时", "使用", duration);
                    }
                    break;
                case Action.STOPPED:
                    Firebase.getInstance(context).logEvent( "VPN状态", "断开成功");
                    if(duration > 0){
                        Firebase.getInstance(context).logEvent( "VPN计时", "断开", duration);
                    }
                    break;
                case Action.ERROR:
                    sharedPreferences = DefaultSharedPrefeencesUtil.getDefaultSharedPreferences(context);
                    errorCount = sharedPreferences.getInt(SharedPreferenceKey.CONNECT_ERROR_COUNT, 1);
                    Firebase.getInstance(context).logEvent( "VPN状态", "错误", String.valueOf(errorCount));
                    sharedPreferences.edit().putInt(SharedPreferenceKey.CONNECT_ERROR_COUNT, errorCount + 1).commit();
                    if(duration > 0){
                        Firebase.getInstance(context).logEvent( "VPN计时", "错误", duration);
                    }
                    break;
            }
        }
